...
from utils.menu_functions import (main_menu, read_duckyscript, Keyboard_real_time_input, run, restart_bluetooth_daemon,
                                  get_target_address)
...
def process_duckyscript(client, duckyscript, current_line=0, current_position=0):
    ...
    char_dir = {' ': 'SPACE', '[': 'LEFTBRACE', ']': 'RIGHTBRACE', ';': 'SEMICOLON', "'": 'QUOTE', '/': 'SLASH',
                '.': 'DOT', ',': 'COMMA', '|': 'PIPE', '-': 'MINUS', '=': 'EQUAL'}
    ...
    try:
        for line_number, line in enumerate(duckyscript):
            ...
            if line.startswith("PRIVATE_BROWSER"):
                report = bytes(
                    [0xa1, 0x01, Modifier_Codes.CTRL.value | Modifier_Codes.SHIFT.value, 0x00, Key_Codes.n.value, 0x00,
                ...
                for char_position, char in enumerate(text, start=1):
                    ...
                    try:
                        ...
                        # Fixed
                            ...
                            key_code = getattr(Key_Codes, char_dir[char])
                            client.send_keypress(key_code)
...
def main():
    ...
    parser.add_argument('--adapter', type=str, default='hci0',
                        help='Specify the Bluetooth adapter to use (default: hci0)')
